...
void random_spin_state(double (&theta)[L][L][L], double (&phi)[L][L][L]){
    for (int l = 0; l < L; l++) {
        for (int m = 0; m < L; m++) {
            for (int n = 0; n < L; n++) {
                double ang1 = random_ZeroTwo() * pi;
                double ang2 = random_ZeroTwo() * pi;
        ...
        for (int m = 0; m < L; m++) {
            for (int n = 0; n < L; n++) {
                for (int o = 0; o < 2; o++) {
                    ...
                    E += (cos(theta[l][m][n]) * sin(phi[l][m][n])) *
                         (cos(theta[near_n[l][o]][m][n]) * sin(phi[near_n[l][o]][m][n]));
                    E += cos(theta[l][m][n]) * sin(phi[l][m][n]) * cos(theta[l][near_n[m][o]][n]) *
                         sin(phi[l][near_n[m][o]][n]);
                    E += cos(theta[l][m][n]) * sin(phi[l][m][n]) * cos(theta[l][m][near_n[n][o]]) *
                         sin(phi[l][m][near_n[n][o]]);
                    E += (sin(theta[l][m][n]) * sin(phi[l][m][n])) *
                         (sin(theta[near_n[l][o]][m][n]) * sin(phi[near_n[l][o]][m][n]));
                    E += (sin(theta[l][m][n]) * sin(phi[l][m][n])) *
                         (sin(theta[l][near_n[m][o]][n]) * sin(phi[l][near_n[m][o]][n]));
                    E += (sin(theta[l][m][n]) * sin(phi[l][m][n])) *
                         (sin(theta[l][m][near_n[n][o]]) * sin(phi[l][m][near_n[n][o]]));
